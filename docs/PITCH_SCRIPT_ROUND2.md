...
| Wouldn't AI-written code just not have these bugs? | No, because this isn't a code bug. The loop in the demo is correct — `zero_grad`, forward, `backward`, `optimizer.step`, nothing wrong with it. What fails is the *learning rate*, a number, and whether 5.0 is too aggressive depends on this model, this batch size, this data order — none of which is visible by reading the code, AI-written or not. An AI assistant suggesting a learning rate does the exact same thing a human copying one from a paper does: pattern-matches a number that worked somewhere else, with no way to simulate whether it diverges on your specific setup short of actually running it. And we have direct proof code quality isn't the variable: sweep 6 in `SWEEP_LOG.md` ran *identical* code, identical seed, identical everything, twice — and got a 62-point spread from floating-point nondeterminism alone. If the exact same code can't even agree with itself run to run, "better-written code" was never going to be the fix. This is a runtime monitoring problem, not a code-generation problem — it lives one layer below the source file, in what the optimizer actually does once the script is running. |
